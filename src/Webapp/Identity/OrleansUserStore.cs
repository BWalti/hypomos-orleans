namespace Webapp.Identity
{
        ...
        IUserAuthenticationTokenStore<ApplicationUser>
    {
        ...
        public Task<string> GetTokenAsync(ApplicationUser user, string loginProvider, string name,
            CancellationToken cancellationToken)
        {
            var tokens =
                user.AuthenticationTokens.Where(token =>
                    (token.Name == name) && (token.LoginProvider == loginProvider));
            return tokens.Count() == 0
                ? Task.FromResult<string>(null)
                : Task.FromResult(user.AuthenticationTokens
                    .Where(token => (token.Name == name) && (token.LoginProvider == loginProvider)).First().Token);
        }

        public Task RemoveTokenAsync(ApplicationUser user, string loginProvider, string name,
            CancellationToken cancellationToken)
        {
            var tokensToRemove = user.AuthenticationTokens
                .Where(token => (token.LoginProvider == loginProvider) && (token.Name == name)).ToList();
            ...
            CancellationToken cancellationToken)
        {
            ...
            user.AuthenticationTokens.Add(
                new AuthTokenState {Token = value, LoginProvider = loginProvider, Name = name});
            return Task.FromResult(0);
        }
        ...
        {
            user.ExternalLogins.Add(new ExternalLoginState
            {
        ...
        {
            var indexGrain =
                this.grainClient.GetGrain<IGrainIndexGrain<IUserGrain>>($"login-{loginProvider}-{providerKey}");
            var userGrain = await indexGrain.Get();
            if (userGrain == null)
            {
                return null;
        ...
        {
            var result = user.ExternalLogins.Select(login =>
                new UserLoginInfo(login.LoginProvider, login.ProviderKey, login.LoginProvider)).ToList();
        ...
        public Task RemoveLoginAsync(ApplicationUser user, string loginProvider, string providerKey,
            CancellationToken cancellationToken)
        {
            user.ExternalLogins.Remove(user.ExternalLogins.First(login =>
        ...
        public async Task<IdentityResult> CreateAsync(ApplicationUser user, CancellationToken cancellationToken)
        {
            ...
            if (user.UserId != null)
            {
                ...
                {
                    Code = "already_exists",
                    Description = $"Can't create a user that already has an id ({user.UserId})"
                });
            }
            ...
            if (user.Registered != null)
            {
                return IdentityResult.Failed(new IdentityError
                {
                    ...
                    Description = $"Can't create a user that is already registered (at {user.Registered})"
                });
            }
            ...
            try
            {
                ...
                var userNameIndexGrain =
                    this.grainClient.GetGrain<IGrainIndexGrain<IUserGrain>>($"username-{user.NormalizedUserName}");
                await userNameIndexGrain.Set(userGrain);

                var emailIndexGrain =
                    this.grainClient.GetGrain<IGrainIndexGrain<IUserGrain>>($"email-{user.NormalizedEmail}");
                await emailIndexGrain.Set(userGrain);

                foreach (var login in registerAccount.IdentityState.ExternalLogins)
                {
                    var loginIndexGrain =
                        this.grainClient.GetGrain<IGrainIndexGrain<IUserGrain>>(
