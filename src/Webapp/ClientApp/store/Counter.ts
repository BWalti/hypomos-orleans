...
interface RequestCounterAction {
    type: "RequestCounterAction"
}

interface ReceiveCounterAction {
    type: "ReceiveCounterAction",
    payload: Server.CounterState
}

// Declare a 'discriminated union' type. This guarantees that all references to 'type' properties contain one of the
// declared type strings (and not any other arbitrary string).
type KnownAction = RequestCounterAction |
                   ReceiveCounterAction |
                   Server.CounterStartedAction |
                   Server.CounterStoppedAction |
                   Server.DecrementCounterAction |
                   Server.IncrementCounterAction |
                   Server.StartCounterAction |
                   Server.StopCounterAction |
                   Server.SyncCounterStateAction;
...
export const actionCreators = {
    request: (): AppThunkAction<KnownAction> => (dispatch, getState) => {
        ...
        var state = getState();
        const fetchTask = fetch(`/counterstate?id=${state.session.id}`,
                {
                    credentials: "include",
                    headers: {
                        'Content-Type': "application/json"
