...
import { HeadCollector } from "./lib/react-head";
...
export default createServerRenderer(params => {
    return new Promise<RenderResult>((resolve, reject) => {
        ...
        params.domainTasks.then(() => {
                ...
                if (!routerContext.url
                ) { // && (!routerContext.status || (routerContext.status >= 200 && routerContext.status < 300)))

                    // Now that we have the state we can collect the head tags
                    const headTags: React.ReactElement<any>[] = [];
                    const appString = renderToString(<App headTags={headTags}/>);

                    resolve({
                        html: renderToStaticMarkup(
                            <Html headTags={headTags} appString={appString} state={store.getState()}/>),
                        statusCode: routerContext.status
                    });
                } else {
                    resolve({
