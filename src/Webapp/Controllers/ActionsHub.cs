namespace Webapp
{
    ...
    public class ActionsHub : Hub
    {
        ...
        public ActionsHub(IClusterClient grainClient, IHttpContextAccessor httpContextAccessor,
            ILogger<ActionsHub> logger)
        {
            ...
            if (string.IsNullOrEmpty(sessionCookie) || !Guid.TryParse(sessionCookie, out this.sessionId))
            {
                this.logger.LogError(
                    "SignalR Hub: unexpected request without expected 'SESSION' cookie. Session will not work!");
        ...
        public override async Task OnConnectedAsync()
        {
            ...
            this.logger.LogInformation(
                $"connection {this.Context.ConnectionId} opened by {this.Context.User.Identity.Name}");
            ...
            var actionsSubscription = await actionsStream.SubscribeAsync(
                ...
                async (action, st) =>
                {
                    ...
                    {
                        this.logger.LogError(e, "Exception sending {0} action to client over SignalR!",
                            action.GetType().Name);
                    }
                });
        ...
        public override async Task OnDisconnectedAsync(Exception exception)
        {
            ...
            {
                this.logger.LogInformation(
                    $"connection {this.Context.ConnectionId} disconnected with exception {exception.Message}");
            }
            else
            {
            ...
            if (this.Context.Items.TryGetValue("actionsSubscription", out var subscription))
            {
