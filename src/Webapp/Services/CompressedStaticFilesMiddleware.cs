namespace Webapp.Services
{
    ...
    {
        private static readonly Dictionary<string, string> compressionTypes = new Dictionary<string, string>
        {
            {"gzip", ".gz"},
            {"br", ".br"}
        };
        ...
        public CompressedStaticFilesMiddleware(RequestDelegate next, IHostingEnvironment hostingEnv,
            IOptions<StaticFileOptions> staticFileOptions, ILoggerFactory loggerFactory)
        {
            ...
            this._logger = loggerFactory.CreateLogger<CompressedStaticFilesMiddleware>();
            ...
            staticFileOptions.Value.FileProvider =
                ...
                foreach (var compressionType in compressionTypes.Keys)
                {
                    ...
                    if (ctx.File.Name.EndsWith(fileExtension, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (contentTypeProvider.TryGetContentType(
                            ctx.File.PhysicalPath.Remove(ctx.File.PhysicalPath.Length - fileExtension.Length,
                                fileExtension.Length), out contentType))
                        {
            ...
            this._base = new StaticFileMiddleware(next, hostingEnv, staticFileOptions, loggerFactory);
        }
        ...
        public Task Invoke(HttpContext context)
        {
            if (context.Request.Path.HasValue)
            {
                ...
                var browserSupportedCompressionTypes = context.Request.Headers["Accept-Encoding"].ToString()
                    .Split(new[] {',', ' '}, StringSplitOptions.RemoveEmptyEntries);
                var orginalFilePath = Path.Combine(this._hostingEnv.WebRootPath,
                ...
                if (orginalFile.Exists)
                {
                    ...
                    if (matchedFile != null)
                    {
                        ...
                        this._logger.LogInformation(
                            $"Request: {context.Request.Path.Value}, matched: {matchedPath}, ({matchedFile.Length} instead of {orginalFile.Length} bytes)");
                        context.Request.Path = new PathString(matchedPath);
                        return this._base.Invoke(context);
                    }
                }
            }
